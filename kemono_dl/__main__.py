...
def parse_args():
    ...
    # NAS Features
    parser.add_argument("--delete-extracted-types", metavar="EXT", type=str, help="NAS: 删除解压后的指定类型文件，多个类型用逗号分隔 (例如: txt,url)")
    parser.add_argument("--no-auto-extract", action="store_true", help="NAS: 禁用自动解压功能")
    parser.add_argument("--clear-failed-marks", action="store_true", help="NAS: 清除所有永久跳过标记，重新尝试下载和解压")
...
def main() -> None:
    ...
    # NAS args processing
    delete_extracted_types = []
    if args.delete_extracted_types:
        delete_extracted_types = [s.strip().lower() for s in args.delete_extracted_types.split(",")]
    ...
    kemono_dl = KemonoDL(
        ...
        no_tmp=args.no_tmp,
        # NAS args passed to KemonoDL
        auto_extract=not args.no_auto_extract,
        delete_extracted_types=delete_extracted_types,
        clear_failed_marks_flag=args.clear_failed_marks
    )
