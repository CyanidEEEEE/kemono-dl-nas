...
from .utils import compute_sha256, generate_file_path, get_sha256_hash, get_sha256_url_content, extract_archive, process_existing_archives, clear_failed_marks
...
class KemonoDL:
    ...
    def __init__(
        ...
        delete_extracted_types: list = [],
        clear_failed_marks_flag: bool = False
    ) -> None:
        ...
        self.delete_extracted_types = delete_extracted_types
        
        # NAS Initialization
        if clear_failed_marks_flag:
            clear_failed_marks(self.path)
            
        if self.auto_extract:
            process_existing_archives(self.path, self.delete_extracted_types)
    ...
    def download_post_attachments(self, domain: str, creator: Creator, post: Post) -> None:
        ...
        for attachment in post.attachments:
            ...
            # --- NAS Feature: Check if already extracted ---
            if self.auto_extract and template_variables.file_ext.lower() in ['zip', '7z', 'rar']:
                ...
                if os.path.exists(hash_file):
                    try:
                        with open(hash_file, 'r', encoding='utf-8') as f:
                            ...
                            if expected_sha256 in hash_data:
                                print(f"[info] 跳过下载: {os.path.basename(file_path)} | 压缩包已被解压过")
                                continue
                    except:
                        pass
            # -----------------------------------------------
            ...
            # --- NAS Feature: Auto Extract after download ---
            if self.auto_extract and template_variables.file_ext.lower() in ['zip', '7z', 'rar']:
                extract_archive(file_path, expected_sha256, self.delete_extracted_types, is_new_download=True)
            # ------------------------------------------------
